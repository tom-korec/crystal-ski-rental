...
| #   | Commit                            | Contents                                                                                                                                                                                                                                                                                                                                                                                                                                               | Size |
| --- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---- |
| 16  | `Add landing page and sign-in`    | Landing with the day/dusk hero and sign-in / sign-up panel, app shell (logo, role-based navigation added as pages land, mobile sheet, role badge, theme switcher, sign out), guarded customer and staff areas, shared `Field`, `FormError` and `PageHeader`. Auth calls bypass the streaming tRPC link so session cookies reach the browser.                                                                                                           | M    |
| 17  | `Add ski search and booking`      | Ski search with every filter and the sort in the URL (parsed with the router schema, invalid parts dropped), secondary filters behind a toggle on small screens, ski cards with the discounted quote, show-more grid, and a reserve dialog with the price breakdown, next-tier hint and store address, contacts and opening hours. Money formatted from decimal strings; date ranges normalised so server and browser render the same text (FR-30…35). | L    |
| 18  | `Add my reservations and ratings` | My reservations as page-numbered cards with status, price paid and store details dialog; cancel with confirmation; rental and ski rating dialogs with a star input on native radios, optional staff-only text, and create / edit-until / reopen / locked states from `rating-rules`. Booking confirmation links to the page (FR-40…44).                                                                                                                | M    |
| 19  | `Add front desk`                  | Front desk with a tab per store (kept in the URL), pickups and returns due today and the overdue ones, each row with the customer's e-mail, inventory code, skis, dates and price, and the actions the lifecycle allows: mark picked up, mark returned, or cancel a no-show after confirming. Refreshes every minute (FR-50…51).                                                                                                                       | M    |
| 20  | `Add fleet management`            | Fleet grid with inventory-code search and catalogue filters in the URL, add-ski dialog, ski detail with model price and rating, edit (store locked while bookings stand), take out of rental with a warning about bookings that stay valid, delete refused while booked, and the paginated reservation history. The reservation row with lifecycle actions is shared with the front desk (FR-20…24, FR-60, BR-22, BR-30…32).                           | L    |
| 21  | `Add catalogue management`        | Admin-only catalogue with tabs in the URL: models (attributes, price, rating and ski counts, ratings dialog with comments and reply-by-email links), brands, and stores (address, phone, e-mail, seven opening-hours fields with "copy Monday"). Add, edit and delete dialogs show the server's in-use refusals; prices are validated without throwing on malformed input; phone and zip formatting is shared (FR-10…14, BR-42).                       | M    |
| 22  | `Add accounts and profile`        | Account list (search, role filter, removed view, page in the URL), create and edit dialogs with the role control for admins only and an optional password reset, account page with remove / restore gated by `mayManageAccount` and the customer's reservation history, and a profile page for every role (rename, change password) reached from the name in the header (FR-4, FR-60…63).                                                              | M    |

### Part D — Quality

| #   | Commit                 | Contents                                                                                                                                                                                                                                                                                                                                                                                                                                  | Size |
| --- | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| 23  | `Add end-to-end tests` | Playwright suite on its own seeded database and port: sign-in, redirects by role, sign-up and phone-width landing; customer booking with the discount and cancelling, both ratings including reopening, and the no-cancel rule once started; front desk through every list; fleet add, withdraw, delete and the booked-ski refusals; admin catalogue, stores and a staff account a manager cannot edit. 15 tests, run one file at a time. | L    |
| 24  | `Add CI workflow`      | GitHub Actions on pushes to `main` and pull requests: a quality job (lint, typecheck, format, unit tests, production build) and an e2e job against a `postgres:17` service, uploading the Playwright report on failure. Requiring the checks on `main` is a repository setting, listed with the deployment steps.                                                                                                                         | M    |
| 25  | `Write README`         | Project overview, setup, demo accounts and what the seed shows, scripts, stack, layout, the decisions behind the code (roles, UTC days, the overlap constraint, exact money, pure rules, privacy, deletion, URL state, errors, theme, tRPC cookies, translations), testing and CI.                                                                                                                                                        | M    |
